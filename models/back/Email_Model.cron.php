...
class Email_Model
{
	...
	public function getMessageByMessageId($messageId)
	{
	    ...
	    try
	    {
	       $query = 'SELECT em.message_id, DATE_FORMAT(em.date, "%Y, %b %e") AS date,
                    TIME_FORMAT(em.hour, "%H:%i") AS hour, n.first_name, 
                    em.message_system_id,
                    n.last_name, personal_name, 
                    em.from_email, em.to_email, em.subject, em.message, em.attachment, 
                    em.status, em.folder, em.user_sender, n.member_id
                    FROM email_messages em
                    LEFT JOIN emails e ON e.email = em.from_email
                    LEFT JOIN names n ON n.member_id = e.member_id
	...
	*/
	...
	public function saveAttachments($email_key, $from, $name)
    {
        try
        {
            $query = 'INSERT INTO email_attachments(message_system_id, email, attachment_name)
                    VALUES("'.$email_key.'", "'.$from.'", "'.$name.'" )';
	...
	public function countInboxByEmailAccount($email_id)
	{
		try
		{
			$query = 'SELECT COUNT(*)
				FROM email_messages em
				LEFT JOIN user_emails se ON em.to_email = se.email
				WHERE se.email_id = '.$email_id.' AND em.folder = 1';
	...
	public function countOutboxByEmailAccount($email_id)
	{
		try
		{
			$query = 'SELECT COUNT(*)
				FROM email_messages em
				LEFT JOIN user_emails se ON em.from_email = se.email
				WHERE se.email_id = '.$email_id.' AND em.folder = 2';
	...
	public function countArchivedByEmailAccount($email_id)
	{
		try
		{
			$query = 'SELECT COUNT(*)
				FROM email_messages em
				LEFT JOIN user_emails se ON em.to_email = se.email
				WHERE se.email_id = '.$email_id.' AND em.folder = 5';
